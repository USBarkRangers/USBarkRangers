...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Firebase setup can return after merely registering the Auth observer.
        // On weak cell service the first Auth callback may still be silent even
        // though setup reported "ready". Whenever identity remains unresolved,
        // rebuild the visit projection from the last durable server baseline,
        // then overlay orange additions and removals. Auth later approves or
        // removes that account-scoped projection.
        if (window._authStateResolved !== true) {
            const authService = window.BARK.services && window.BARK.services.auth;
            const offlinePremiumSession = firebaseBootResult.status === 'timeout'
                && authService && typeof authService.activateOfflinePremiumSession === 'function'
