...
(function () {
    ...
    function createSavedVisitStartup() {
        ...
        function restore() {
            ...
            // Local editing uses the same account guard and grace as saved pins.
            // Firebase keeps running; this grants no server confirmation.
            if (window._authStateResolved !== true) {
                try {
                    window.BARK.services?.auth?.activateOfflinePremiumSession?.(rememberedUid);
                } catch (error) {
                    console.warn('[B.A.R.K. Boot] Offline access unavailable; saved-pin recovery continues.', error);
                }
            }
            const service = checkin();
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (window._authStateResolved !== true && savedVisitStartup.isAccountEligible()) {
            ...
            if (firebaseBootResult.status === 'timeout'
                ...
                authService.activateOfflinePremiumSession(window.BARK.services?.checkin?.getRememberedAuthenticatedVisitUid?.());
            }
        }
