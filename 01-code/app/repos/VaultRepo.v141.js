...
(function () {
    ...
    function isAuthoritativeSnapshot(metadata = {}) {
        // Missing metadata is not proof. Only Firestore's explicit server
        // metadata may confirm an orange mutation or replace the trusted base.
        return metadata.fromCache === false && metadata.hasPendingWrites === false;
    }

    function isPersistedBaselineSnapshot(metadata = {}) {
        return metadata.persistedBaseline === true;
    }

    function mergeMissingVisits(baseVisits, incomingVisits) {
        const nextVisits = new Map(baseVisits);
        incomingVisits.forEach((visit, id) => {
            if (!nextVisits.has(id)) nextVisits.set(id, cloneVisit(visit));
        });
        return nextVisits;
    }
    ...
    function confirmPendingUpsert(expectedVisit) {
        const placeId = getVisitId(expectedVisit);
        if (!placeId || !expectedVisit || typeof expectedVisit !== 'object') return false;

        const mutation = pending.get(placeId);
        if (!mutation || mutation.type !== 'upsert' || !mutation.place) return false;
        const currentVisit = visits.get(placeId);
        if (!recordsMatch(currentVisit, expectedVisit)
            || !recordsMatch(mutation.place, expectedVisit)) {
            return false;
        }

        pending.delete(placeId);
        commit('confirmPendingUpsert', new Map(visits), {
            pendingChanged: new Set([placeId]),
            skipNoop: true
        });
        return true;
    }
    ...
    function reconcileSnapshot(visitsArray, metadata = {}) {
        const incomingVisits = cloneMap(visitsArray);
        const persistedBaseline = isPersistedBaselineSnapshot(metadata);
        const freshAuthoritative = isAuthoritativeSnapshot(metadata) && !persistedBaseline;
        const snapshotCanConfirm = freshAuthoritative && metadata.canConfirmPending !== false;
        let nextVisits;

        if (freshAuthoritative) {
            // Fresh server evidence owns the complete base and may acknowledge
            // exact pending mutations.
            nextVisits = incomingVisits;
            trustedBaselineSeen = true;
        } else if (persistedBaseline && !trustedBaselineSeen) {
            // A previously server-proven baseline is safe to display after a
            // reload. It owns the complete historical base, while the pending
            // overlay below preserves every newer local intent. It must never
            // confirm a mutation staged in the current runtime.
            nextVisits = incomingVisits;
            trustedBaselineSeen = true;
        } else if (!trustedBaselineSeen) {
            // Before any trusted base is available, cached data can fill gaps
            // for offline startup. It cannot remove or overwrite newer runtime
            // state because cache metadata is not server ordering proof.
            nextVisits = mergeMissingVisits(visits, incomingVisits);
        } else {
            // Once a trusted base exists, delayed cache or latency-compensated
            // snapshots cannot regress it. Local writes are already represented
            // by the pending overlay applied below.
            nextVisits = new Map(visits);
        }
    ...
    function normalizeSnapshotMetadata(doc) {
        const metadata = doc && doc.metadata ? doc.metadata : null;
        return {
            fromCache: metadata && typeof metadata.fromCache === 'boolean'
                ? metadata.fromCache
                : undefined,
            hasPendingWrites: metadata && typeof metadata.hasPendingWrites === 'boolean'
    ...
    function handleVisitedSnapshot(uid, doc, options = {}) {
        ...
        if (isAuthoritativeSnapshot(metadata)) {
            const checkpointSaved = callOptionalCallback(
                'rememberAuthoritativeVisitIds',
                options.rememberAuthoritativeVisitIds,
                uid,
                placeList
            );
            // Exact server data is necessary but not sufficient to retire the
            // orange journal: its complete baseline must also survive local
            // storage. Missing/throwing callbacks therefore fail closed.
            metadata.canConfirmPending = checkpointSaved === true;
        }
