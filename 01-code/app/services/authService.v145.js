...
let userSnapshotWatchdog = null;
const USER_SNAPSHOT_WATCHDOG_MS = 15000;
    ...
    return {
        ...
        rememberAuthoritativeVisitIds(uid, visits) {
            ...
            if (checkinService && typeof checkinService.rememberAuthoritativeVisitIds === 'function') {
                return checkinService.rememberAuthoritativeVisitIds(uid, visits);
            }
            return false;
        },
        onChange(change) {
            if (hasAuthoritativeSnapshotMetadata(change && change.metadata)
                && change.metadata.canConfirmPending !== false) {
...
function reconcileVaultRepoFromUserSnapshot(user, data, metadata = {}) {
    ...
    if (hasAuthoritativeSnapshotMetadata(normalizedMetadata)
        ...
        normalizedMetadata.canConfirmPending = options.rememberAuthoritativeVisitIds(user.uid, placeList) === true;
    }
...
function stopUserSnapshotSubscription() {
    if (userSnapshotWatchdog !== null) {
        clearTimeout(userSnapshotWatchdog);
        userSnapshotWatchdog = null;
    }
...
function activateOfflinePremiumSession(expectedUid = null) {
    ...
    const session = premiumService.restoreOfflineSession(expectedUid);
        ...
        firebase.auth().onAuthStateChanged((user) => {
            try {
                ...
                if (user) {
                    ...
                    // Rebuild the display from the last durable server baseline
                    // even when Auth returned quickly but the user-document
                    // listener is still blackholed by weak cellular service.
                    // The helper overlays additions; deletion replay below wins.
                    ...
                    try {
                        ...
                        userSnapshotUnsubscribe = firebase.firestore().collection('users').doc(user.uid)
                            .onSnapshot({ includeMetadataChanges: true }, (doc) => {
                                try {
                                    ...
                                    if (!doc.metadata.fromCache && userSnapshotWatchdog !== null) {
                                        clearTimeout(userSnapshotWatchdog);
                                        userSnapshotWatchdog = null;
                        ...
                        userSnapshotWatchdog = setTimeout(() => {
                            userSnapshotWatchdog = null;
                            const currentUser = firebase.auth().currentUser;
                            if (!currentUser || currentUser.uid !== user.uid || window._firstServerPayloadReceived) return;
                            if (window.BARK && typeof window.BARK.showOfflineRecoveryNotice === 'function') {
                                window.BARK.showOfflineRecoveryNotice(
                                    'Account data is still waiting for service. Reload here to restart the connection.'
                                );
                            }
                        }, USER_SNAPSHOT_WATCHDOG_MS);
                    } catch (error) {
