...
function removeVisitedEntries(entries) {
    ...
    const checkinService = window.BARK.services && window.BARK.services.checkin;
    const offlineSession = !authenticatedUser && window._authStateResolved !== true
        ? checkinService?.getActiveOfflinePremiumVisitSession?.()
        : null;
    const user = authenticatedUser || offlineSession;
    ...
    // Offline intent stays in the existing account journal. Auth replay owns
    // the first cloud write once this identity is restored.
    const syncPromise = authenticatedUser ? syncUserProgress() : Promise.resolve(null);
