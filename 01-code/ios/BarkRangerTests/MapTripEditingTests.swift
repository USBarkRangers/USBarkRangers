...
@MainActor struct MapTripEditingTests {
    ...
    @Test func heldDragCanChangeDaysAndDropAtomicallyWithoutLosingStopFields() async throws {
        ...
        let store = try await NativeStore.open(directory: directory, project: "demo-bark-native", uid: "user")
        ...
        let result = try #require(try await store.currentNativeDraft(id: trip.id)?.trip)
        ...
        let reopened = try await NativeStore.open(
            directory: directory, project: "demo-bark-native", uid: "user")
        #expect(try await reopened.currentNativeDraft(id: trip.id)?.trip == result)
