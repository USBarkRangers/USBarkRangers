...
public struct NativeVisitPage: Codable, Equatable, Sendable {
    ...
    public let items: [NativeVisitRecord]
    ...
    public func validate(after cursor: Cursor?) throws {
        ...
        if let next {
            guard items.count == 50, next == previous else { throw NativeRecordValidation.Failure.malformed }
        }
    }
}
