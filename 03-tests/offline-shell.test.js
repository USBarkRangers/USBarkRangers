...
const MANIFEST_PATH = path.join(APP_ROOT, 'offline', 'cacheManifest-0.145.js');
const PRIOR_MANIFEST_PATH = path.join(APP_ROOT, 'offline', 'cacheManifest-0.141.js');
const SW_PATH = path.join(APP_ROOT, 'sw.js');
const HTML_PATH = path.join(APP_ROOT, 'index.v145.html');
...
test('the worker uses the physical 0.145 manifest and private entry', () => {
    const manifest = loadCacheManifest();
    const sw = fs.readFileSync(SW_PATH, 'utf8');
    assert.equal(manifest.entry, './index.v145.html');
    assert.match(sw, /cacheManifest-0\.145\.js/);
