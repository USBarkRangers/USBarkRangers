...
1. Extend the existing Debug-only AppSandbox/explicit composition boundary with synthetic account/provider adapters before adding live auth providers; keep the isolated test-host and UI-test storage paths. Create dedicated emulator configuration and an idempotent seed tool that refuses non-emulator hosts/projects. Seed synthetic account A/B, free/Premium/expired/manual/Lemon states and representative current-format records. Emulator Functions must use provider test adapters that cannot send real emails, support messages, charges or cancellations.
