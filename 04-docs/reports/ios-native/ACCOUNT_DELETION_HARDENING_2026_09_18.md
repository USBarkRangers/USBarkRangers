...
- Storage ownership decides deletion. Account data kept in the account's `NativeStore`, or under the account's files folder that `NativeAccountRemovalFiles.eraseClosedAccount` removes, is erased with no hook, at launch, before any feature object exists. Put new account data there and there is nothing to forget. That folder has one definition, `NativeAccountRemovalFiles.accountFiles(directory:uid:)`, which `RecordingStore`, `FeedbackDraftStore` and the erase itself all call; before September 19 each derived the path by hand. Never compute it yourself: a path derived by hand is a path deletion cannot see. A test pins the folder name so the formula cannot change and orphan files already on phones. The `eraseAdditionalAccountData` closure in `AppComposition` is only for what cannot live there: in-memory buffers, shared caches such as the route cache, and stores with their own root such as saved places. Two guards cover its wiring. `AppShellTests` asserts the composition installs it, which catches a refactor before release. At run time `AccountSession.completeRemoval` refuses to finish without it: cleanup fails with the usual warning and retry, the marker and the account folder stay, and a build that wires the hook finishes the job. A missing hook can no longer report the device clean. Test fixtures that keep nothing outside the account folder install an empty hook and say so. On the server, data under `users/{uid}` is deleted automatically; anything outside it must be added to the list in `functions-native/accounts/deletion.js`.
...
## Known future architecture items

Deliberately not done. Nothing here is broken today.

- **Shared rejection-reason definitions (T3).** Server rejection reasons are strings copied into five or more client files. Do it with the next sync or backend rejection work, not alone. Until then the server must not send a new reason to shipped apps: an unknown reason stops that queue.
- **Strict profile enums (T3).** No change. The rule above stands: ship a tolerant decode before adding a map style or status.
- **Left alone unless already editing those lines (T4):** a `scopeStarted` guard so cleanup cannot run beside an open account; `tripLibraryMessage` also carrying visit, walk and handoff messages, and `reportOpenFailure`'s default giving a new feature the visit-or-walk text; provider IDs as raw strings; the unused `AccountSession.feature(_:)` and `AccountAssembly.nativeProfile(app:uid:)`; the three session hooks being reassignable; `routeDay.stop()` where a fuller `resetScope()` exists; a saved-pins failure closing the whole scope; the dense `admitted`.
- **Outside architecture (T2, Platform):** no account failure leaves the phone.
