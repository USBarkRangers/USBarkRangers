...
September 10, 2026. Current build **0.2.16 (18)** progressively reveals detail content during the low-to-medium drag. Version 0.2.15 (17) prevents map-loading errors during zoom/press from falsely switching to offline overview. Version 0.2.14 (16) removes the measured half-second wait before pin selection. Version 0.2.13 (15) matches sheet entry/exit timing and removes sheet-driven camera movement. Version 0.2.12 (14) prevents a popup-dismissal tap from seeding native double-tap/one-finger zoom. Version 0.2.11 (13) adds a brisk downward park-detail dismissal. Version 0.2.10 (12) makes individual park badges about 10% smaller while retaining their touch targets and selected enlargement. Version 0.2.9 (11) keeps a stable low/medium selected-pin position, enlarges selected pins slightly and uses a brisk reversible search/tab transition. It retains 0.2.8, which corrects the returning tab-bar position, prevents ungrouped pin collision hiding, isolates development/test storage and removes marker rescans caused only by result order. It retains 0.2.7 sheet-versus-content scrolling, coordinated search/tab travel and local-preference corrections. It retains 0.2.6 native camera gliding, grouping-setting fixes, low/medium browsing height and lower selected-pin placement. It retains the 0.2.5 selection, background-result, marker-invalidation and catalog-diagnostic corrections. Original 0.2.0 implementation commit `4084569`, verification/string-catalog follow-up `ba8aa68`, on `codex/ios-native-setup`, GitHub destination `USBarkRangers/USBarkRangers`. Phase 2 implementation is complete and is **awaiting user testing**, with the verification limits below. User acceptance remains pending. Phase 3 has not started.
...
## Progressive low-to-medium detail reveal — 0.2.16 (18)

Previously, moving one point above low replaced the compact presentation with all medium content. The sheet now derives one clamped expansion fraction from its existing low/medium heights. Name expansion occupies the first 30% of travel, metadata/tags reveal over 15–55%, and thumbnails over 45–100%. These overlapping stages ease their boundaries and reverse from the current height when the drag reverses. Working action buttons remain present throughout.

The existing `ParkDetailView` owns one private [SwiftUI Layout](https://developer.apple.com/documentation/swiftui/layout) that interpolates actual content height, including long names and Dynamic Type. It crossfades the compact/full title using the same interpolated Dynamic Type font size in both variants, keeping their shared first line aligned; rows reveal with matching height, opacity and spacing. Only one title is exposed to accessibility, and hidden rows cannot receive input. Snapping interpolates one expansion fraction through the view’s native Animatable conformance; title text disables independent implicit glyph reflow, which otherwise briefly overlapped wrapped words during the snap. Direct dragging follows the finger without an added timer or delayed reveal task. Reduce Motion retains the existing immediate snap behavior. No new runtime file, dependency, measured-height cache or persistent state is introduced. The detail view and its private layout stay together in 130 lines; the consolidated metadata view is 31 lines.

Four existing presentation files change: `ParkSheetLayout.expansion` derives progress, `ParkDetailSheet` forwards it, `ParkDetailView` renders it, and `ParkDetailMetadata` places category, distinct source type, state, swag and cost in one horizontal tag row. A source type matching its category is shown once; the separate metadata subtitle is removed. Tags keep their subtle system background, with minimum height reduced from 32 to 26 points and side padding from 10 to eight; text can still grow for accessibility sizes. Detents, scroll ownership, search/tab motion, map camera, catalog and preferences are unchanged. High content remains scrollable only after reaching high. No Phase 3 work is included.

Verification:

- The new rendered regression samples 0%, 2%, 15%, 40%, 70%, 100%, then reverses to 40%/0%. Actual scroll-content height grows progressively, the first small lift does not install the whole medium layout, reverse heights match, and vertical offset stays zero. Captures use an attached test window and assert rendering succeeds.
- All five native presentation tests pass on the final code (`/tmp/BarkProgressiveSnapPro.xcresult`), including nested horizontal scrolling, tab restoration and annotation reuse. The five unchanged native camera/grouping tests passed earlier in `/tmp/BarkProgressivePro.xcresult`. All **11 domain tests pass** (`/tmp/bark-progressive-domain.log`).
- Final three-detent UI checks pass on **iPhone 17 Pro and iPhone SE 3** / iOS 26.5, including short partial lifts returning low, unchanged selected-pin position, actions, high details, the shared category/state tag row and no duplicate National tag. Evidence: `/tmp/BarkProgressiveSnapPro.xcresult`, `/tmp/BarkProgressiveSnapSE.xcresult`. Body-drag/scroll ownership and largest accessibility text passed with the consolidated slim tags in `/tmp/BarkProgressiveTagsPro.xcresult` and `/tmp/BarkProgressiveTagsSE.xcresult`.
- The six existing Pro sheet UI scenarios also passed before the final title/tag refinements, including dark landscape, exposed-map dismissal and repeated high-scroll returns. An initial SE test used a lift that crossed its snap threshold; the partial-lift fixture now uses 20/40 points and passes on both phone sizes. No detent change was made to satisfy that fixture.
- Inspected actual rendered intermediate heights and the final recorded drag/snap (`/tmp/bark-progressive-snap-ui.mov`), correcting overlapping text reflow before completion. Build with Swift warnings treated as errors and strict formatting/diff checks pass. Physical-device motion feel remains a user acceptance check.

Normal Debug build **0.2.16 (18)** is installed and open on the iPhone 17 Pro simulator (`/tmp/bark-progressive-normal-build.log`).

User check: slowly lift from low, pause, reverse, then continue to medium. The name should open before the compact tag row and photos, while Directions/Park Info stay available. Metadata such as National, Maine, Tag and $$ belongs in one horizontal row. No catalog/data/backend changes or Phase 3 work.

## False offline fallback during zoom — 0.2.15 (17)

The map delegate previously interpreted **every** `mapViewDidFailLoadingMap` error as unavailable imagery. That latched state installed the opaque offline basemap, even with a connected network path, and stayed set until reconnection or a map-appearance change. Apple's [delegate documentation](https://developer.apple.com/documentation/mapkit/mkmapviewdelegate/mapviewdidfailloadingmap(_:witherror:)) explicitly allows this callback when another tile request arrives while a previous one is pending. A rapid zoom/press can therefore expose a map-loading error without the device going offline.

The correction removes the error-to-offline callback, the extra `imageryUnavailable` state and its appearance/reconnect reset paths. Overview now has two inputs: observed network-path unavailability or the user's explicit Overview preference. MapKit keeps ownership of connected imagery/loading. This removes **8 runtime lines net across two existing files**; no new file, timer, retry service, gesture interception or map preference is added. Selection, gestures, camera timing, catalog architecture and query/annotation invalidation are unchanged.

A connected path does not prove that Apple's service or the wider internet is reachable. When that service cannot load while the path remains available, MapKit keeps its existing imagery and the user can still choose Overview manually. This deliberately avoids treating a tile error as evidence of a lost connection. Catalog freshness continues to require a validated HTTP response independently.

Verification:

- A regression injected cancellation, timeout, MapKit throttling and provider-server errors into the actual delegate boundary. It failed against the old runtime with eight assertions (incorrect offline state/installed overlays), then passed after the fix. Selection, result identity, annotation version and camera remain unchanged; a real disconnected/connected transition still adds/removes offline geography without another result computation.
- All **50 app test functions passed** (41 Swift Testing + 9 native XCTest), including appearance/reconnect/manual Overview, selection cancellation and annotation invalidation. All **11 domain tests passed**.
- The real dismissal/pan/double-tap UI scenario now runs with an isolated connected-path fixture and presses the map after intentional zoom. It verifies that overview does not appear, deliberate zoom still works, dismissal does not seed accidental zoom, panning still works, and query/count remain intact. This passed on **iPhone 17 Pro and iPhone SE 3** / iOS 26.5. The intermittent Apple-provider error itself was reproduced by deterministic callback injection, not by claiming control of live tile delivery.
- **393 / 5,000 records:** 300 geometry updates each still cause zero marker lookups/add/remove calls. Background projection medians were **3.21–4.39 ms / 14.36–33.58 ms**, with MainActor heartbeat progress in every sample. These simulator regression checks are not a physical-device performance guarantee.
- Debug build with warnings treated as errors and strict formatting/diff checks passed. Evidence: `/tmp/BarkMapOfflineRed.xcresult`, `/tmp/BarkMapOfflineGreen.xcresult`, `/tmp/BarkMapOfflineSE.xcresult`, `/tmp/bark-map-offline-build.log`, `/tmp/bark-map-offline-domain.log`. Physical-device airplane-mode/provider timing and minimum-supported iOS remain acceptance checks.

Normal Debug build **0.2.15 (17)** is installed and open on the iPhone 17 Pro simulator. Its build passed with warnings treated as errors (`/tmp/bark-map-offline-normal-build.log`).

User check: rapidly double-tap to zoom, then press/drag the map and select another park. Apple imagery should remain visible while connected. Test explicit Overview and an actual disconnect/reconnect separately. No Phase 3, web, catalog or backend changes.

## Faster pin response — 0.2.14 (16)

A temporary passive touch probe measured five pin taps before and after the correction on iPhone 17 Pro / iOS 26.5. Before, the native `didSelect` callback arrived **501.5–502.8 ms after finger release**, followed by the camera command another 3–6 ms later. After, the completed tap reached the selection action in **0.39–0.41 ms**, with the camera command **2.07–3.01 ms after release**. Median release-to-camera-command time fell from **506.63 ms to 2.16 ms**. These are command timings, not a measurement of display scan-out or physical-device latency. Evidence: `/tmp/bark-pin-latency-baseline.log`, `/tmp/bark-pin-latency-after.log`, `/tmp/BarkPinLatencyBefore.xcresult`, `/tmp/BarkPinLatencyAfter.xcresult`.

The existing map tap recognizer now accepts individual park annotations and forwards a completed tap directly to the same authoritative selection model. The existing public UIKit gesture-priority policy consumes selection taps as well as dismissal taps. It still exempts continuous pan/pinch/rotation, excludes clusters/controls, and preserves ordinary background double-tap zoom. A weak annotation reference identifies only the current touch; it is cleared on completion/next touch and is not a second selected park. No selection occurs on finger-down, so dragging from a pin remains a pan. Native `didSelect` remains available for clusters/accessibility. The only runtime edit is **13 additional lines in the existing 198-line MapCoordinator**. All temporary probes/logging were removed. No extra recognizer, file, timer, settings preference or camera animation change ships.

Comparison of the checked-out web implementations (BARK at `82fe026`; Junior Ranger working tree based on `6158e62`, including its existing local edits; neither web tree changed):

| Interaction | BARK web | Junior Ranger web | Native 0.2.14 |
|---|---|---|---|
| Selection path | Marker click directly opens panel and pans | Same direct click path | Completed pin tap directly selects; removes the measured native callback wait |
| Pin-to-pin pan | 0.5 seconds, Leaflet ease-out | 0.5 seconds, Leaflet ease-out | Existing 0.3-second native ease-in/out context retained |
| Panel entry | 0.35 seconds, cubic-bezier(0.25, 0.8, 0.25, 1) | Same entry curve; mobile height uses a 0.34-second overshooting curve | Existing 0.26-second matching entry/exit retained |
| Selected unvisited artwork | 1.30 scale with upward lift | SVG grows from 32×44 to 39×54 px (about 22–23%), with a thicker yellow ring | Existing 1.12 scale retained |

The web pan keeps zoom fixed and offsets the selected coordinate above the mobile panel by 180 px. Both reference Leaflet 1.9.4; the bundled pan implementation defaults to quadratic ease-out when no easeLinearity override is supplied. It begins promptly and eases into the landing, whereas native ease-in/out has a gentler initial acceleration. BARK also defines a 0.3-second overshooting hover response; Junior Ranger’s current image-pin path swaps to the larger SVG and stronger yellow ring. My interpretation of the more playful feel is immediate acknowledgement plus more pronounced pin feedback and a quick-start/soft-landing curve—not a shorter overall animation. This patch removes the measured input wait first, preserving the motion the user just approved.

Source references: BARK `01-code/app/renderers/panelRenderer.js` (smart auto-pan), `styles.css` (slide-panel) and `styles/mapStyles.css` (active-pin/hover); matching files plus `MapMarkerConfig.js` in `/Users/carterswarm/JrRangerMapCode/01-code/app/`; Leaflet 1.9.4 `panBy`/`PosAnimation` in BARK's bundled vendor source. Uses Apple's public [tap recognizer](https://developer.apple.com/documentation/uikit/uitapgesturerecognizer) and [gesture failure-priority delegate](https://developer.apple.com/documentation/uikit/uigesturerecognizerdelegate/gesturerecognizer(_:shouldberequiredtofailby:)).

Verification: **9 native map/presentation tests and 11 domain tests passed**. Four iPhone 17 Pro UI scenarios passed: grouping changes, search collapse/restore, pin switching/remembered detents/fixed zoom, and dismissal-tap isolation/pan/double-tap zoom. The strengthened drag-from-pin and pin-switching check passed on both Pro and SE 3; dismissal/zoom also passed on SE. The first SE drag check touched a pin where the floating filter rail intercepted the gesture. The final test pans the pair below the controls and asserts that precondition before dragging, then requires camera movement without accidental selection. No runtime correction was needed for that test placement.

Debug builds passed with Swift warnings treated as errors; strict formatting/diff checks passed. Evidence: `/tmp/BarkPinResponseFinal.xcresult`, `/tmp/BarkPinResponseProExposed.xcresult`, `/tmp/BarkPinResponseSEExposed.xcresult`, `/tmp/bark-pin-response-se-final.log`, `/tmp/bark-pin-response-domain.log`, `/tmp/bark-pin-response-normal-build.log`. Normal build **0.2.14 (16)** is installed and open on iPhone 17 Pro for testing. Physical-device motion feel and minimum-supported iOS remain user acceptance checks. No Phase 3, web, catalog or backend changes.

## Stable camera and matching sheet transitions — 0.2.13 (15)

Park details now enter and leave with the same **0.26-second ease-in/out** bottom transition; Reduce Motion fades both. Dismissal clears the old chrome height so opening the next card cannot inherit a hidden-search/tab state. Low/medium browsing memory and the outgoing high card remain intact.

The camera owner no longer receives the live sheet position or height, changes native map margins, or clears its remembered selection at high. Those paths allowed high-to-medium to reframe and changed MapKit's viewport during pin switching. Only a new selected identity, corrected park coordinate or explicit camera request can issue another framing command. Search obstruction measures only the search bar/chips, excluding the disappearing results dropdown. Sheet movement and geometry updates cannot command a camera correction.

New pin selections use native `setCenter`, preserving zoom/heading, within a **0.3-second ease-in/out animation context** that allows interaction and starts from the current state. MapKit retains gesture/interruption ownership. Reduce Motion repositions immediately. No custom animation loop, delayed callback, timer, duplicate selection, new preference or new runtime file is introduced. The four existing runtime files lose **eight lines net**; the framing owner is 44 lines.

Verification:

- All **49 app test functions** passed (40 Swift Testing + 9 native XCTest), including rapid successive selection commands, Reduced Motion, unchanged camera/margins through geometry updates, corrected-coordinate handling, grouping and native ungrouped visibility. All **11 domain tests** passed.
- Five focused iPhone 17 Pro UI scenarios passed: pin-to-pin selection/remembered detents/unchanged zoom, repeated high-scroll-to-medium returns, exposed-high-map dismissal/reselection, dismissal-tap zoom isolation and search-collapse/restore. High-to-medium now asserts the selected pin stays within one point of its prior position on every return. A recorded pin switch was inspected frame by frame for the rendered glide; native animation duration remains under MapKit's control rather than a custom camera engine.
- The pin-to-pin/remembered-detent scenario and repeated high-scroll-to-medium check also passed on iPhone SE 3, for **seven focused UI runs** across both phone sizes. Evidence: `/tmp/BarkSheetCameraSE.xcresult`.
- At **393 and 5,000 records**, 300 geometry updates and reordered results with unchanged membership still produce **zero marker lookups/add/remove calls**. Background projection medians were **3.06–4.28 ms / 13.54–29.50 ms**, with MainActor heartbeat progress throughout. These are simulator regressions, not physical-device performance certification.
- Debug build with warnings treated as errors and strict formatting/diff checks passed. Evidence: `/tmp/BarkSheetCameraUI.xcresult`, `/tmp/BarkSheetCameraRegression.xcresult`, `/tmp/bark-sheet-camera-domain.log`, `/tmp/bark-sheet-camera-ui.mov`, `/tmp/bark-sheet-camera-final-build.log`. Normal build **0.2.13 (15)** is installed and open on the iPhone 17 Pro simulator for testing.

User check: select another pin during a glide or map pan; expand high, scroll, return medium/low, then close/reopen. The card should enter/exit at matching speed, selection should glide without zooming or vertical corrections, and sheet movement should leave the map alone. Physical-device motion feel and minimum-supported iOS remain acceptance checks. No Phase 3 work or backend/data architecture changes are included.

## Dismissal tap and native zoom — 0.2.12 (14)

`MapCoordinator` captures the background touch's dismissal intent before clearing selection. A public UIKit dynamic failure requirement gives that tap priority over competing map gestures for this recognition attempt. Public pan, pinch and rotation recognizers are exempt, so continuous navigation starts normally. Pin/control touches stay excluded. A new map touch resets the transient intent; this is not a second selection authority, saved setting or timer. Normal unselected-map taps continue sharing recognition with MapKit, so deliberate native zoom remains available. Native time/distance thresholds are not changed, and no private gesture class or map subtree is inspected in production.

The regression sends a genuine synthesized double tap without an idle wait between its two touches. Before the fix, dismissing details also increased the distance between two pins from about 146 to 240 points. The final check requires unchanged scale, immediate ordinary panning with/without selection, correct deselection, and successful deliberate double-tap zoom afterward. Search/counts must remain unchanged.

Verification: Debug build passed with Swift warnings treated as errors; the new regression passed on iPhone 17 Pro and iPhone SE 3. Existing search-collapse/restore and high-sheet map-dismissal/reselection UI checks passed on Pro (four final UI runs total). All 11 domain tests and strict formatting passed. Evidence: `/tmp/BarkDismissGestureBefore.xcresult`, `/tmp/BarkDismissGestureVerified.xcresult`, `/tmp/BarkDismissGestureSEVerified.xcresult`, `/tmp/bark-dismiss-gesture-domain.log`. Normal build 0.2.12 (14) is installed on Pro for user testing. Exact one-finger tap-then-held-drag timing remains a physical-device acceptance check; the automated regression proves the shared first-tap cause and preserved pan/double-tap behavior.

Uses Apple's documented [dynamic gesture failure requirements](https://developer.apple.com/documentation/uikit/uigesturerecognizerdelegate/gesturerecognizer(_:shouldberequiredtofailby:)). The runtime change is contained in the existing 185-line coordinator; no new runtime file, catalog/query work or settings preference is added.

## Brisk park-detail dismissal — 0.2.11 (13)

`MapScreen` scopes a 0.26-second ease-in removal transition to its sheet overlay. Tapping exposed map or Close clears the authoritative selection immediately while SwiftUI slides the outgoing card down. Reduce Motion uses opacity. High resets to low only when the next park is selected, so dismissal never shrinks the outgoing high card. Low/medium browsing memory remains unchanged. No timer, delayed selection mutation, duplicate Park state, new file or model change.

Verification: Debug build passed with Swift warnings treated as errors; both existing UI checks for exposed-map dismissal/reselection and the full three-detent/search-restoration path passed on iPhone 17 Pro. The recorded high exit was inspected frame by frame: the entire card travels down with its contents intact. All 11 domain tests and strict formatting passed. Evidence: `/tmp/BarkSheetDismissFinal.xcresult`, `/tmp/bark-sheet-dismiss-final.log`, `/tmp/bark-sheet-dismiss-domain.log`, `/tmp/bark-sheet-dismiss-final.mov`. Installed build 0.2.11 (13) for user testing; physical-device motion feel remains a user check.
